...
src/
  ...
  components/
    ui/                 Links, labels, meta lists, development notes
...
5. `/projects/[slug]` resolves a visible project by slug. Hidden and production-sample projects 404.

Components should not contain names, bios, or project copy.

Production builds omit `sample` projects, template experience, placeholder skills, and developer notes (`process.env.NODE_ENV === "development"`).
...
The visual identity is an editorial engineering portfolio: compact top navigation, a name-led hero, full-width project bands, a cool paper About section, and `#58D68D` as a signal rather than a theme.
...
`src/config/sections.ts` is a flat list of `{ id, enabled, order }`. `src/app/page.tsx` maps ids to section components and skips anything disabled or empty. Navigation is filtered in `src/lib/navigation.ts` so Experience disappears when there are no public roles.
...
- **Dark theme with a paper exception.** About inverts to ink on cool off-white. Tokens are named so a second theme could be added later.
